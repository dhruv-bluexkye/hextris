function render() {
	...
	// Update timer display in high score position - only show during gameplay
	if (gameTimer !== undefined) {
		if (gameState == 1 && gameTimerStartTime !== undefined) {
			...
			$("#currentHighScore").text(timerValue);
			// Change color to red when 5 seconds or less
			if (gameTimer <= 5) {
				$("#currentHighScore").css("color", "#e74c3c");
			} else {
