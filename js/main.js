...
function set_score_pos() {
    ...
    var target_midpoint = (trueCanvas.height / 2 + igt_bottom)/2
...
function resumeGame() {
	...
	importing = 0;
	startTime = Date.now();
	gameTimer = gameTimerDuration; // Reset timer to configured duration when starting
	gameTimerStartTime = Date.now(); // Track when timer started
	gameStartTime = Date.now(); // Track when game actually started for Flutter integration
...
function init(b) {
	...
	// Initialize timer display - hide it initially (will show when game starts)
	$("#highScoreInGameText").hide();
	if (gameTimer !== undefined) {
		$("#currentHighScore").text(Math.ceil(gameTimer));
	} else {
	...
	startTime = Date.now();
	gameTimer = gameTimerDuration; // Reset timer to configured duration when initializing
	gameTimerStartTime = Date.now(); // Track when timer started
	gameStartTime = Date.now(); // Track when game actually started for Flutter integration
...
function animLoop() {
	...
	case 1:
		...
		if (gameState == 1 && gameTimer <= 0 && !importing) {
...
function showHelp() {
	...
	if(gameState != 0 && !infobuttonfading) {
		return;
	}
